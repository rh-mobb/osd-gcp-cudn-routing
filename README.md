...
**`make create`** runs these steps in order (same as the former three-command quick start):
...
2. **`make bgp.deploy-controller`** — [**`controller_gcp_iam/`**](controller_gcp_iam/README.md) apply, WIF credential **Secret** (**`gcloud`** ADC), **ConfigMap** populated from **`cluster_bgp_routing` `terraform output`**, then namespace / RBAC / ImageStream / BuildConfig / Deployment, **binary image build**, and rollout ([`scripts/bgp-deploy-controller-incluster.sh`](scripts/bgp-deploy-controller-incluster.sh)). Pass **`TF_VARS`** / **`EXTRA_TF_VARS`** on the **`make`** command line when needed; they are forwarded to **`bgp.run`** / **`bgp.deploy-controller`**.

3. **`make bgp.e2e`** — CUDN pod ↔ echo VM **`ping`** / **`curl`**. Test pods use the default scheduler (no router **nodeAffinity**; every worker is a BGP peer). Wait until Cloud Router BGP is **Established** if the first run is flaky.

You can run **`make bgp.run`**, **`make bgp.deploy-controller`**, and **`make bgp.e2e`** separately instead of **`make create`**.
...
Convention: **`stack.action`** separated by dots; multi-word segments use hyphens (for example **`controller.gcp-iam.init`**, **`controller.deploy-openshift`**). **`make create`** / **`make destroy`** wrap the [quick start](#quick-start--bgp). **`bgp.apply`** is Terraform-only for **`cluster_bgp_routing/`** (unlike **`bgp.run`**, which includes WIF, **`oc login`**, and **`configure-routing.sh`**).

| Target | Directory / action |
|--------|-------------------|
| `create` / `destroy` | **`bgp.run`** + **`bgp.deploy-controller`** + **`bgp.e2e`** / **`bgp.destroy-controller`** + **`bgp.teardown`** (full reference lifecycle) |
