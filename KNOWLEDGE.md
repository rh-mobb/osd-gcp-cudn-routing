...
- **Virt e2e [`scripts/e2e-virt-live-migration.sh`](scripts/e2e-virt-live-migration.sh) Phase A** (after internal mesh + echo) probes **Internet** (ping to **`VIRT_E2E_INTERNET_PING_HOST`**, curl **`VIRT_E2E_INTERNET_CURL_URL`**) from netshoot, both guest VMs, the icanhazip pod, and the echo VM when available. **`VIRT_E2E_ALLOW_INTERNET_FAIL=1`** (default) only **warns** on failure, because **CUDN → Internet** is often **ECMP-unreliable** in this reference architecture (see *CUDN internet egress* elsewhere in this file); set **`0`** or **`--strict-internet`** to **fail the script** on a bad probe. **Confidence: 90%** (intentional test behavior + existing egress notes).
- **Phase B (MIG2 / MIG3) concurrent ping and curl** during a live migration default to the **Terraform echo client VM** (**`gcloud compute ssh`**, `--tunnel-through-iap`) when **`VIRT_E2E_MIG_CONCURRENT_FROM=auto`** and the usual echo outputs + **gcloud** are present, so the under-test path matches the **spoke → CUDN (BGP) echo stack**; the script falls back to **netshoot** otherwise, and MIG3’s icanhazip **expected body** is **the echo VM’s internal IP** in the first case and **netshoot’s CUDN pod IP** in the second. **MIG2** from the echo VM uses **`ping -i 0.2`**, not `0.1` — non-root Linux enforces a **≥200ms** minimum between probes. **`=netshoot`** or **`=echo`** force a side. **Confidence: 80%** (intended to align Phase B with the e2e-cudn/VPC path; re-check if Terraform is absent or the echo leg is disabled).
- **Ad-hoc SSH to virt-e2e guests:** use **`netshoot-cudn`** on the same CUDN as a jump. The manual path is the same as **`virt-ssh.sh`**: **`oc cp`** the private key into **`netshoot-cudn`**, then **`oc exec` … `ssh -i /tmp/virt-e2e-vm-key … cloud-user@<vmi-guest-IPv4>`** (IP from **`oc get vmi`**, prefer CUDN from **`cudn_cidr`** if multiple addresses). [`scripts/virt-ssh.sh`](scripts/virt-ssh.sh) wraps that (interactive, or **`-- <command>`** in **`BatchMode`** for scripts). The matching **public** key must be in **`authorized_keys`** from cloud-init in [`scripts/e2e-virt-live-migration.sh`](scripts/e2e-virt-live-migration.sh). [AGENTS.md](AGENTS.md) (§ *CUDN / KubeVirt guests: SSH*) lists the exact **`oc cp` / `oc exec`** order. The virt e2e **Phase A** mesh uses this jump for **VM-to-VM** and **VM-to-icanhazip-cudn** HTTP checks. **`make virt.ssh.bridge`** / **`make virt.ssh.masq`** use **`-C cluster_bgp_routing`**. Re-run after **netshoot** is recreated. If **`Permission denied (publickey)`** after regenerating **`id_ed25519`**, the guest disk may still have an old **`authorized_keys`** — recreate the VM or fix keys on the guest. **Confidence: 95%** (aligned with [docs/networking-validation-test-plan.md](docs/networking-validation-test-plan.md) §2.1 and virt e2e probes).
...
- **In-cluster deploy RBAC (`operator/deploy/rbac.yaml`) must grant `machine.openshift.io` `machines` (`get`, `list`, `watch`, `patch`, `update`).** Without it, controller-runtime’s Machine informer fails at cluster-scoped list/watch, logs repeat `forbidden: User "…/bgp-routing-operator" cannot list resource "machines"`, cache sync times out, and the manager exits (`CrashLoopBackOff`). **`make manifests` only regenerates rules backed by `+kubebuilder:rbac` markers** — hand-edited `config/rbac/role.yaml` entries are dropped on the next generate unless the marker exists on a reconciler. **Confidence: 95%** — reproduced from live pod logs (Apr 2026) and `controller-gen` behavior on this repo.
