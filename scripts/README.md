...
| [`bgp-destroy.sh`](bgp-destroy.sh) | Destroy **`cluster_bgp_routing/`** then **`wif_config/`**. Invoked by **`make bgp.teardown`** (second step of **`make destroy`**). Run **`make bgp.destroy-controller`** first if you used **`make bgp.deploy-controller`** but are not using **`make destroy`** (cleans controller-managed GCP/OpenShift state and **`controller_gcp_iam/`**), or **`make controller.cleanup`** only if you want to keep the controller IAM stack. |
| [`bgp-deploy-controller-incluster.sh`](bgp-deploy-controller-incluster.sh) | After **`make bgp.run`**: **`controller_gcp_iam`** apply, WIF credential Secret, ConfigMap from **`cluster_bgp_routing`** outputs, OpenShift manifests (no **`deploy/configmap.yaml`**). Without **`BGP_CONTROLLER_PREBUILT_IMAGE`**: ImageStream, BuildConfig, binary **`oc start-build`**, **`oc rollout restart`**, **`oc rollout status`**. With **`BGP_CONTROLLER_PREBUILT_IMAGE`** (e.g. **`make create`**): RBAC + Deployment only, skip build, rollout. Invoked by **`make bgp.deploy-controller`**. |
| **`make bgp.e2e`** | Run [`e2e-cudn-connectivity.sh`](e2e-cudn-connectivity.sh) with **`-C cluster_bgp_routing/`** (from repo root). After **`make bgp.run`** and **`make bgp.deploy-controller`** (and after BGP-labeled nodes are **Ready** — **`make create`** / **`make dev`** print **`post-controller-deploy-msg`** with **`watch 'oc get nodes -l …'`** instead of running e2e). Requires **`oc`** and **`gcloud`**. |
| **`make bgp.phase1-baseline`** | [references/fix-bgp-ra.md](../references/fix-bgp-ra.md) **Phase 1**: no cluster changes — [`bgp-phase1-baseline.sh`](bgp-phase1-baseline.sh) prints nodes labeled **`cudn.redhat.com/bgp-router`**, **`RouteAdvertisements/default`** (**`spec.nodeSelector`**), **`FRRConfiguration`** in **`openshift-frr-k8s`**, then **`cluster_bgp_routing/scripts/debug-gcp-bgp.sh`**. Flags: **`--skip-gcp`**, **`--e2e`**. |
...
|----------|---------|---------|
| `OC_LOGIN_EXTRA_ARGS` | _(empty)_ | Extra args to `oc login` (e.g. `--insecure-skip-tls-verify`). If this includes **`--insecure-skip-tls-verify`**, the API TLS wait below is skipped. |
