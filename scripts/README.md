...
| [`bgp-deploy-controller-incluster.sh`](bgp-deploy-controller-incluster.sh) | After **`make bgp.run`**: **`controller_gcp_iam`** apply, WIF credential Secret, ConfigMap from **`cluster_bgp_routing`** outputs, OpenShift manifests (no **`deploy/configmap.yaml`**), binary **`oc start-build`**, **`oc rollout restart`** on the Deployment (so **`…:latest`** picks up the new image), then **`oc rollout status`**. Invoked by **`make bgp.deploy-controller`**. |
| **`make bgp.e2e`** | Run [`e2e-cudn-connectivity.sh`](e2e-cudn-connectivity.sh) with **`-C cluster_bgp_routing/`** (from repo root). After **`make bgp.run`** and **`make bgp.deploy-controller`**, with **`oc`** and **`gcloud`** working. |
