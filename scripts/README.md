...
| Script | Purpose |
|--------|---------|
| [`networking-validation-test.sh`](networking-validation-test.sh) | **Orchestrated** networking checks: [`e2e-cudn-connectivity.sh`](e2e-cudn-connectivity.sh) (optional second namespace), [`e2e-virt-live-migration.sh`](e2e-virt-live-migration.sh) with **`--run-tests`** by default (migrations + netshoot probes; **`--virt-hints-only`** for deploy + console hints only), optional **`--internet-probes`**. See **[docs/networking-validation-test-plan.md](../docs/networking-validation-test-plan.md)**. **`make networking.validate`**. |
