...
| Script | Purpose |
|--------|---------|
| [`networking-validation-test.sh`](networking-validation-test.sh) | **Orchestrated** networking checks: [`e2e-cudn-connectivity.sh`](e2e-cudn-connectivity.sh) (optional second namespace), [`e2e-virt-live-migration.sh`](e2e-virt-live-migration.sh) with **`--run-tests`** (mesh, echo, migrations; **`--virt-hints-only`** = virt **`--access-only`**) and optional **`--internet-probes`**. See **[docs/networking-validation-test-plan.md](../docs/networking-validation-test-plan.md)**. **`make networking.validate`**. |
...
| [`bgp-destroy.sh`](bgp-destroy.sh) | Destroy **`cluster_bgp_routing/`** then **`wif_config/`**. Invoked by **`make bgp.teardown`** (last step of **`make destroy`**). **`make destroy`** also runs **`virt.destroy-storage`** first, then **`bgp.destroy-operator`**. If you are not using **`make destroy`**, run **`virt.destroy-storage`** (optional if you never deployed virt), then **`make bgp.destroy-operator`** before **`make bgp.teardown`** when you used **`make bgp.deploy-operator`**. |
| [`bgp-deploy-operator-incluster.sh`](bgp-deploy-operator-incluster.sh) | After **`make bgp.run`**: **`controller_gcp_iam`** apply, WIF credential Secret, CRDs from **`operator/config/crd/bases/`**, **`operator/deploy/`** RBAC, **`BGPRoutingConfig`** `cluster` from Terraform outputs, ImageStream + BuildConfig + **`oc start-build`** from **`operator/`** (unless **`BGP_OPERATOR_PREBUILT_IMAGE`**), Deployment with WIF volumes. Invoked by **`make bgp.deploy-operator`**. **`make create`** / **`make dev`** run this script then **`make virt.deploy`**. |
...
| `OC_LOGIN_RETRY_MAX_SEC` | `600` | After the TLS wait (if any), retry **`oc login`** until success or this many seconds total (admin password may not be accepted immediately after apply). |
| `OC_LOGIN_RETRY_INTERVAL_SEC` | `20` | Seconds between **`oc login`** attempts when credentials are not yet accepted. |
...
| Script | Purpose |
|--------|---------|
| [`destroy-openshift-virt-storage.sh`](destroy-openshift-virt-storage.sh) | Before cluster teardown: delete **all** **VirtualMachineInstanceMigration** and **VirtualMachine** objects cluster-wide and wait for **VMI** teardown (**`VIRT_DESTROY_WAIT_SEC`**); then **os-images** CDI importers (**DataImportCron** / **DataVolume**), **VolumeSnapshots**, **PVCs** on **`sp-balanced-storage`**; then **VolumeSnapshotClass** **`csi-gce-pd-vsc-images`**, **StorageClass** **`sp-balanced-storage`**, restore **`standard-csi`** default; then **GCP disks** still in the pool and **`STORAGE_POOL_NAME`** in **`virt_storage_zone`** (bare metal AZ when the BM pool is enabled), or each **`availability_zones`** entry if that output is absent. **`make virt.destroy-storage`**. Env: **`SKIP_GCP_POOLS=1`**, **`SKIP_CLUSTER_STORAGE=1`**, **`OS_IMAGES_NS`**, **`VIRT_DESTROY_WAIT_SEC`**. Does **not** remove **`openshift-cnv`** / CNV. |
| [`deploy-openshift-virt.sh`](deploy-openshift-virt.sh) | **Preflight:** validates GCP Hyperdisk pool sizing (capacity / IOPS / throughput vs [pool limits](https://cloud.google.com/compute/docs/disks/storage-pools#pool-limits)), **`oc apply --dry-run=client`** for **StorageClass** + CNV OLM + **HyperConverged**, and **`curl`** HEAD on the VolumeSnapshotClass URL. **CNV:** creates **OperatorGroup** only if the namespace has none (avoids **TooManyOperatorGroups** vs OperatorHub **`openshift-cnv-*`**); fails if **two+** **OperatorGroups** exist. **Then:** pool (**`gcloud compute storage-pools create`**), **StorageClass**, **`standard-csi`**, **VolumeSnapshotClass**, OLM, **HyperConverged**, verify. Reads **`gcp_project_id`**, **`gcp_region`**, **`virt_storage_zone`** (else first **`availability_zones`** entry) from **`cluster_bgp_routing/`**. **`make virt.deploy`**. Storage: [gcp-storage-configuration-4.21](https://github.com/noamasu/docs/blob/main/gcp/gcp-storage-configuration-4.21.md). |
| [`e2e-virt-live-migration.sh`](e2e-virt-live-migration.sh) | After **`virt.deploy`** and **`configure-routing.sh`**: by default **Phase A** ( **`deploy-cudn-test-pods.sh`**, netshoot + **icanhazip** ↔ each VM, VM↔VM HTTP over **in-guest** SSH via the jump, optional **echo** path as in [`e2e-cudn-connectivity.sh`](e2e-cudn-connectivity.sh), then **Internet**: ping **8.8.8.8** and curl **https://www.google.com** from netshoot, icanhaz, both guests, echo when present — **warn** on failure by default, **`--strict-internet` to fail**). **Phase B** (migrations) is **opt-in** (**`--with-migrations`**, **`--run-tests`**, **`make virt.e2e.full`**). **`--access-only`**. Two **`VirtualMachine`s** on the **default pod network**: **`VIRT_E2E_VM_NAME_BRIDGE`** (**`binding.name: l2bridge`**, same spelling as [rh-mobb/rosa-bgp **`setup-vm-ssh.sh`**](https://github.com/rh-mobb/rosa-bgp/blob/main/tests/setup/setup-vm-ssh.sh), default **`virt-e2e-bridge`**) and **`VIRT_E2E_VM_NAME_MASQ`** (**`masquerade: {}`** / SNAT, default **`virt-e2e-masq`**). Same **`DataVolumeTemplates`** + **`DataSource`** + **`cloudInitNoCloud`** shape as [rh-mobb/rosa-bgp **`test-vm-cudn.yaml`**](https://github.com/rh-mobb/rosa-bgp/blob/main/test-vm-cudn.yaml) but with the cluster **CentOS Stream 9** image. **`VIRT_E2E_VM_NAME`** (or **`--vm-name`**) selects which VM runs **live migration** + **`netshoot-cudn`** ping/curl (defaults to the bridge VM). Boot disk: **`VIRT_E2E_BOOT_DATASOURCE_*`**, **`VIRT_E2E_BOOT_DISK_Gi`**, optional **`VIRT_E2E_BOOT_STORAGE_ACCESS_MODE`**, **`VIRT_E2E_VM_MEMORY`** / **`VIRT_E2E_VM_CPU`**. **`--cleanup`** deletes all VMs labeled **`routing.osd.redhat.com/virt-e2e=true`**. **`make virt.e2e`**. See **[ARCHITECTURE.md](../ARCHITECTURE.md#vm-specific-considerations)** (**masquerade** traffic can be disrupted during migration — expected for the masquerade VM if you migrate it). |
| [`virt-ssh.sh`](virt-ssh.sh) | **`make virt.ssh.bridge`** / **`make virt.ssh.masq`**: interactive **`ssh`** to **`cloud-user`** on the virt-e2e VM via **`netshoot-cudn`** (primary UDN: **`virtctl ssh`** is often unusable). Optional **`-- <command>…`** for non-interactive one-shots (**`BatchMode`**). Requires **`cluster_bgp_routing/.virt-e2e/id_ed25519`** and a Ready **netshoot** pod. Overrides: **`CUDN_NAMESPACE`**, **`VIRT_E2E_VM_NAME_*`**; or **`virt-ssh.sh -C cluster_bgp_routing <vm-name> [-- cmd …]`**. See **[KNOWLEDGE.md](../KNOWLEDGE.md)** and **[docs/networking-validation-test-plan.md](../docs/networking-validation-test-plan.md)** §2.1. |
