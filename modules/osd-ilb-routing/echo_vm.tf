...
# Intermittent failures are under investigation; docs recommend a short retry loop (~5 tries).
...
resource "google_compute_instance" "echo_client" {
  ...
  # which is absent on several GCP Linux images including rhel-9 and centos-stream-9.
  metadata = {
    startup-script = <<-EOT
      #!/bin/bash
      set -euxo pipefail
