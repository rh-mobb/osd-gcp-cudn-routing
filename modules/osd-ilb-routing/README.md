...
- `google_compute_firewall` -- **`${cluster_name}-worker-subnet-to-cudn`** — **all protocols** from the worker subnet CIDR to **`cudn_cidr`** (ingress on ILB workers; echo VM / test VMs → CUDN pods)
...
- (Optional) `google_compute_firewall` -- SSH from 0.0.0.0/0 to the echo VM (ephemeral public IP for direct SSH)
...
**Firewall:** One ingress rule allows `protocol = "all"` from `cudn_cidr` to the tagged instance. Another allows SSH (tcp:22) from 0.0.0.0/0 to the echo VM’s internal IP (destination range `/32`) so the rule matches ingress to the public IP without relying on tags alone. Priority 100 for SSH.
...
**Outputs:** `echo_client_vm_internal_ip`, `echo_client_vm_external_ip`, `echo_client_http_url`, `echo_client_vm_zone`.
