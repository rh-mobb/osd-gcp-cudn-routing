...
// ReconcileNCCSpokes creates/updates numbered spokes and deletes stale ones. Returns count of mutating API calls.
func ReconcileNCCSpokes(ctx context.Context, ncc gcp.NCCClient, cfg *config.ControllerConfig, routerNodes []gcp.RouterNode) (int, error) {
	prefix := cfg.NCCSpokePrefix
	chunks := ChunkRouterNodes(routerNodes, config.NCCMaxInstancesPerSpoke)
