...
// CleanupStaleFRR deletes FRRConfigurations in the FRR namespace with our label that are not in desiredNames.
func CleanupStaleFRR(ctx context.Context, c client.Client, cfg *config.ControllerConfig, desiredNames map[string]struct{}) (int, error) {
...
// ReconcileFRRConfigurations ensures one FRRConfiguration per router node; returns created count (after cleanup).
func ReconcileFRRConfigurations(ctx context.Context, c client.Client, cfg *config.ControllerConfig, routerNodes []gcp.RouterNode, nodeMap map[string]string, topology *gcp.CloudRouterTopology) (created, deleted int, err error) {
